...
/* ── Compute upcoming posts from live calendar state ── */
function computeUpcomingPosts() {
  try {
    ...
    return Object.entries(map)
      ...
      .slice(0, 4)
      .map(([date, post]) => ({ ...post, date }));
  } catch (_) {
    const today = new Date(); today.setHours(0,0,0,0);
    return (window.POSTS_DATA || []).filter(p => new Date(p.date + 'T00:00:00') >= today).slice(0, 4);
  }
}
...
function NextPosts({ visible }) {
  const { useState, useEffect } = React;
  const [upcoming, setUpcoming] = useState(() => computeUpcomingPosts());

  useEffect(() => {
    const handler = () => setUpcoming(computeUpcomingPosts());
    ...
    if (window.SupaDB) {
      window.SupaDB.read('calendar').then(data => {
        ...
        try {
          ...
          try { window.dispatchEvent(new Event('ctax-metrics-update')); } catch (_) {}
        } catch (_) {}
      });
    }
    return () => window.removeEventListener('ctax-metrics-update', handler);
